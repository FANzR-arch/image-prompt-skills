...
每套都符合编译契约：单变量、零选项、否定只在 AVOID。使用时只替换 `"<LATIN>"`（大拉丁词）和 `「<标题>」`（中文小标）两处。要纯拉丁就删掉中文那行；要传播封面就把中文调大、拉丁缩小。

> 所有预设默认叠加 `compile-contract.md` 的**风格纯度护栏**（反 De Stijl / Bauhaus / Op Art / 品牌视觉）。几何向预设（2 黑白形态 / 5 色带 / 8 序列色格 / 9 程序变体 / 6 破格）尤其要让网格和排印明显主导，避免滑成色彩构成或图案板。

---

## 预设 1 · MB 同心弧 / 红（最经典）
配方：MB + E1 + K1 + 标题顶部 + 4:5 + TC
```text
...
## 预设 2 · Hofmann 黑白形态（高张力·无彩色）
配方：HOF + E7 + K5 + 标题底部 + 4:5 + TC
```text
...
## 预设 3 · Ruder 字体即图 / 黄黑（出图最稳）
配方：RUD + E4 + K3 + 标题阶梯错位 + 16:9 + TC
```text
...
## 预设 4 · der Film 纯网格 / 黑底红（编辑感·信息密度）
配方：MB + E5 + K4 + 标题中段 + 16:9 + TC
```text
...
## 预设 5 · Max Bill 色带（结构/层次主题）
配方：BILL + E3 + K6(cyan) + 标题顶部 + 4:5 + TC
```text
...
## 预设 6 · Weingart 受控破格 / 米白橙（设计·实验主题）
配方：WEIN + E7 + K6(orange) + 标题竖排 + 4:5 + TC
```text
