...
- 提示词里允许 AI 自己加一层功能性小字（拉丁标签、字段编号、index/module 标记、细线），这是瑞士信息层级的正常质感，不是噪声——但**主标题必须准确**，次级小字优先用拉丁/抽象字段，少用小中文。
